...
calc_distances <- function(seurat_obj, reduction_name, criteria = "Within_Max",
  ...
  if (any(is.na(dist_matrix))) {
    cli::cli_inform("Distance matrix contains NAs.")
    # Option: Remove NAs or use complete cases only
  }
  if (any(is.na(clusters_int))) {
    cli::cli_inform(c("i" = "Clustering vector contains NAs.") ) # should never happen
  ...
  if (xfun::attr2(dist_matrix, "Size") != length(clusters_int)) {
    cli::cli_inform(c("i" = "Length mismatch between the distance matrix and the clustering."))
  }
  ...
  tryCatch({
    ...
    cli::cli_inform(c("i" = "Error in {.fn cluster.stats}: {e$message}"))
    return(NULL)
  })
