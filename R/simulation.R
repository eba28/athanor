...
#'
#' @description
#' This function simulates an assay of AIRR embeddings by generating a matrix of random values between -0.6 and 0.6, with a specified number of cells and embedding dimensions.
#'
#' @details
...
#'
#' @details
#' It uses both [sim_airr_manual()] and [sim_gex_manual()] to generate the necessary data for the WNN simulations. The function also includes error handling to capture any issues that arise during the simulation runs and provides an option to display a progress bar.
#'
    ...
    tryCatch({
      run_wnn(seurat_obj =
                sim_gex_manual(num_genes = other_vars[["Genes"]],
                               ...
                               separator = "-"),
              embeddings =
                sim_airr_manual(num_cells = other_vars[["Cells"]],
